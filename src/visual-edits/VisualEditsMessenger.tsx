...
// Helper to parse data-kiro-id to extract file path, line, and column
const parseKiroId = (
  kiroId: string
): { filePath: string; line: number; column: number } | null => {
  // Format: "filepath:line:column"
  const parts = kiroId.split(":");
...
export default function HoverReceiver() {
  ...
  const protectChildElements = (element: HTMLElement) => {
    ...
    childElements.forEach((child) => {
      ...
      childEl.setAttribute("data- Net-protected", "true");
  ...
  const restoreChildElements = (element: HTMLElement) => {
    ...
    protectedElements.forEach((child) => {
      ...
      childEl.removeAttribute("data- Net-protected");
  ...
  const handleTextChange = (element: HTMLElement) => {
    ...
    const kiroId = element.getAttribute("data-kiro-id");
    if (!kiroId) return;
    ...
    if (newText !== oldText) {
      const parsed = parseKiroId(kiroId);
      if (!parsed) return;
  ...
  const handleStyleChange = (
    ...
    const existingStyles = appliedStylesRef.current.get(kiroId) || {};
    appliedStylesRef.current.set(kiroId, { ...existingStyles, ...styles });
  ...
  const handleStyleBlur = (element: HTMLElement) => {
    ...
    const appliedStyles = appliedStylesRef.current.get(kiroId);
  ...
  const flushImageSrcChange = () => {
    ...
    const kiroId = imgElement.getAttribute("data-kiro-id");
    if (!kiroId) return;
  ...
  useEffect(() => {
    ...
    const handleMouseUp = () => {
      if (focusedElementRef.current && focusedElementId) {
        ...
        // Extract file info from data-kiro-id
        const kiroId = element.getAttribute("data-kiro-id");
        if (kiroId) {
          const parsed = parseKiroId(kiroId);
          if (parsed) {
  ...
  useEffect(() => {
    ...
    function onPointerMove(e: PointerEvent) {
      ...
      const hit =
        document
          ...
          ?.closest<HTMLElement>("[data-kiro-id]") ?? null;
      ...
      if (hit !== lastHitElementRef.current) {
        ...
        const hitId = hit.getAttribute("data-kiro-id");
        ...
        const tagName =
          hit.getAttribute("data- Net-name") || hit.tagName.toLowerCase();
        ...
        allMatchingElements.forEach((element) => {
          ...
          const elementId = element.getAttribute("data-kiro-id");
    ...
    function onClickCapture(e: MouseEvent) {
      ...
      const target = e.target as HTMLElement;
      const hit = target.closest<HTMLElement>("[data-kiro-id]");
      ...
      if (hit) {
        ...
        // Clicked on empty space or element without data-kiro-id
      ...
      if (e.data.msg === "SHOW_ELEMENT_HOVER" && "elementId" in e.data) {
        ...
        if (allMatchingElements.length > 0) {
          ...
          allMatchingElements.forEach((element) => {
            ...
            if (!tagName) {
              tagName =
                element.getAttribute("data- Net-name") ||
